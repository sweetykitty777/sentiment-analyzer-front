...
export default function UploadAnalytics({ upload }: { upload: UploadFull }) {
  ...
  const [pieData, setPieData] = useState<{sentiment: string, count: number, fill: string}[]>([]);
  ...
  useEffect(() => {
    ...
    const pie = []
    if (positive > 0) {
      pie.push({ sentiment: "positive", count: positive, fill: "var(--color-positive)" });
    }
    if (neutral > 0) {
      pie.push({ sentiment: "neutral", count: neutral, fill: "var(--color-neutral)" });
    }
    if (negative > 0) {
      pie.push({ sentiment: "negative", count: negative, fill: "var(--color-negative)" });
    }
    if (very_positive > 0) {
      pie.push({ sentiment: "very_positive", count: very_positive, fill: "var(--color-very_positive)" });
    }
    if (very_negative > 0) {
      pie.push({ sentiment: "very_negative", count: very_negative, fill: "var(--color-very_negative)" });
    }
    setPieData(pie);

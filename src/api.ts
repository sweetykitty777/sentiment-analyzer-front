...
export const fetchUploadFull = async ({
  client,
  uploadId,
}: {
...
export const getShareUploadsRecipients = async ({
  client,
  uploadId,
}: {
  client: AxiosInstance;
  ...
  const { data } = await client.get<UploadAccess[]>(
    `/uploads/${uploadId}/share`,
  );
  return data;
};

export const deleteShareUpload = async ({
  client,
  uploadId,
  recipientId,
  recipientType,
